      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#1E293B] pb-3 bg-[#161D2A] p-4 rounded-xl shadow-sm">
        ...
        <div className="flex flex-wrap items-center gap-3 text-xs">
          ...
          <div className="bg-[#0B0E14] border border-[#1E293B] px-3 py-1.5 rounded-lg">
            ...
            <span className={`font-bold font-mono-tabular ${wallet && wallet.unrealizedPnL >= 0 ? 'text-[#00C896]' : 'text-[#F6465D]'}`}>
              {wallet ? `${wallet.unrealizedPnL >= 0 ? '+' : ''}$${wallet.unrealizedPnL.toFixed(2)}` : 'No data available'}
            </span>
          </div>

          <div className="bg-[#0B0E14] border border-[#1E293B] px-3 py-1.5 rounded-lg">
            <span className="text-[#94A3B8] block text-[10px]">CHALLENGE (DAY {challenge?.currentDay ?? 1}/20)</span>
            <span className="text-[#3B82F6] font-bold font-mono-tabular">
              {challenge ? `$${challenge.currentBalance.toFixed(2)} / $${(challenge.initialBalance * 1.1).toFixed(2)}` : 'No data available'}
            </span>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between border-b border-[#1E293B] pb-2">
              ...
              <span className="text-[10px] bg-[#00C896]/20 text-[#00C896] px-2 py-0.5 rounded font-bold">
                {latestDecision ? `CONFIDENCE: ${latestDecision.confidenceScore.toFixed(1)}%` : 'No data available'}
              </span>
            </div>
            ...
            <div className="bg-[#0B0E14] border border-[#1E293B] p-3 rounded-lg space-y-2 text-xs">
              <div className="flex justify-between items-center">
                ...
                <span className="text-[#00C896] font-bold">
                  {latestDecision ? latestDecision.decisionState : 'No data available'}
                </span>
              </div>
              <div className="flex justify-between items-center">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="text-[#F59E0B] font-bold">No data available</span>
              </div>
            </div>
            ...
            <div className="bg-[#0B0E14] border border-[#1E293B] p-3 rounded-lg space-y-2 text-xs">
              ...
              <div className="flex justify-between text-[11px]">
                <span className="text-[#94A3B8]">Account Equity:</span>
                <span className="text-white font-bold">${wallet ? wallet.equity.toFixed(2) : '—'}</span>
              </div>
              <div className="flex justify-between text-[11px]">
                <span className="text-[#94A3B8]">Max Risk (1.5%):</span>
                <span className="text-[#F59E0B] font-bold">${wallet ? (wallet.equity * 0.015).toFixed(4) : '—'}</span>
              </div>
              <div className="flex justify-between text-[11px]">
                <span className="text-[#94A3B8]">Symbol:</span>
                <span className="text-[#3B82F6] font-bold">{activeSymbol}</span>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#161D2A] border border-[#1E293B] rounded-xl p-5 max-w-md w-full space-y-4 shadow-2xl font-mono">
            ...
            <div className="bg-[#0B0E14] border border-[#1E293B] p-3 rounded-lg space-y-2 text-xs">
              ...
              <div className="flex justify-between"><span>Decision State:</span><span className="text-white font-bold">{latestDecision?.decisionState ?? 'No data available'}</span></div>
              <div className="flex justify-between"><span>Confidence:</span><span className="text-[#00C896] font-bold">{latestDecision ? `${latestDecision.confidenceScore.toFixed(1)}%` : 'No data available'}</span></div>
              <div className="flex justify-between"><span>Account Equity:</span><span className="text-[#94A3B8]">${wallet ? wallet.equity.toFixed(2) : '—'}</span></div>
              <div className="flex justify-between border-t border-[#1E293B] pt-1"><span>Available Margin:</span><span className="text-[#00C896] font-bold">${wallet ? wallet.availableMargin.toFixed(2) : '—'}</span></div>
            </div>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-[#161D2A] border border-[#1E293B] rounded-xl p-5 max-w-lg w-full space-y-4 shadow-2xl font-mono">
            ...
            <div className="bg-[#0B0E14] border border-[#1E293B] p-3 rounded-lg space-y-2 text-xs">
              ...
              <div className="flex justify-between"><span>Realized PnL:</span><span className="text-white font-bold">${wallet ? wallet.realizedPnL.toFixed(2) : '—'}</span></div>
              <div className="flex justify-between"><span>Unrealized PnL:</span><span className="text-[#F59E0B] font-bold">${wallet ? wallet.unrealizedPnL.toFixed(2) : '—'}</span></div>
              <div className="flex justify-between border-t border-[#1E293B] pt-1"><span>Current Wallet Balance:</span><span className="text-[#00C896] font-bold">${wallet ? wallet.virtualBalance.toFixed(2) : '—'}</span></div>
              <div className="flex justify-between"><span>Challenge Progress:</span><span className="text-[#3B82F6] font-bold">{challenge ? `$${challenge.currentBalance.toFixed(2)} / $${(challenge.initialBalance * 1.1).toFixed(2)}` : 'No data available'}</span></div>
            </div>
