...
export const MarketWatchPanel: React.FC = () => {
  ...
  const pairs = Object.values(mockMarketPairs).filter((p) =>
  ...
  if (!isMarketWatchOpen) {
    return (
      <div className="bg-[#161D2A] border-r border-[#1E293B] flex flex-col items-center py-3 px-1 z-10">
        <button
  ...
  return (
    <aside className="w-64 bg-[#161D2A] border-r border-[#1E293B] flex flex-col z-10 select-none">
      {/* Panel Header */}
            ...
            return (
              <button
                ...
                className={`w-full text-left p-3 transition-colors ${
                  isSelected
                    ? 'bg-[#1E2638] border-l-2 border-[#3B82F6]'
              ...
              >
                <div className="flex items-center justify-between font-mono">
                  ...
                  <span className="text-xs font-bold text-[#F8FAFC]">{pair.price}</span>
                </div>
