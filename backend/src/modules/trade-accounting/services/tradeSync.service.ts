      ...
      if (prisma.tradeLedger?.findMany) {
        ...
        if (dbEntries && dbEntries.length > 0) {
          ...
          const mapped: TradeLedgerEntryDto[] = dbEntries.map((d) => {
            ...
            const closingFee = Number((d.exitPrice * d.quantity * 0.0005).toFixed(4));
            ...
            return {
              ...
              rewardPercent: d.rewardPercent,
              plannedRR: d.riskPercent > 0 ? Number((d.rewardPercent / d.riskPercent).toFixed(2)) : 2.0,
              actualRR: d.riskPercent > 0 ? Number((d.grossPnL / (d.entryPrice * d.quantity * (d.riskPercent / 100))).toFixed(2)) : 0,
              stopLoss: d.stopLoss,
              ...
              lossOffsettingAllowed: true,
              netPnL: d.netPnL,
              roiPercent: d.marginUsed > 0 ? Number(((d.netPnL / d.marginUsed) * 100).toFixed(2)) : 0,
              capitalUtilizationPercent: Number(((d.marginUsed / 10.0) * 100).toFixed(2)),
              durationSeconds: d.durationSeconds,
              durationFormatted: this.formatDuration(d.durationSeconds),
              executionLatencyMs: d.executionLatencyMs,
              decisionConfidence: d.decisionConfidence,
              decisionExplanation: d.decisionExplanation,
              resultStatus: (d.resultStatus as 'WIN' | 'LOSS' | 'BREAKEVEN') || (d.netPnL >= 0 ? 'WIN' : 'LOSS'),
